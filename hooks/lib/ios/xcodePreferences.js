 ...
 * Uses cordova-ios API for project path (backwards compatible with Cordova iOS 5.0+).
 *
...
function loadProjectFile() {
  ...
  try {
    var iosProject = iosProjectHelper.getIosProject(context);
    pbxPath = iosProject.locations.pbxproj;
    ...
    // Fallback: try pre-5.0 / 5.0 cordova structure
  }
...
function pathToEntitlementsFile() {
  var projName = iosProjectHelper.getCordovaProjName(context);
  var fileName = projName + ".entitlements";
  return path.join(projName, "Resources", fileName);
}
